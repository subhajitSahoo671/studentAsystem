    ...
    <div className='w-full min-h-screen px-5 md:px-15 py-5'> 
     ...
     <div>
      ...
      Object.entries(report).map(([Exam, records]) => (
          <div key={Exam} className='my-5 p-3 md:p-5 border border-gray-300 rounded-md shadow-md  w-fit text-amber-50  bg-gradient-to-br from-[#da5fda] via-[#43A9DD] to-[#2a97e6] '>
          ...
          <tbody>
            {records && records.map((data, i) => (
              <tr key={data.studentId._id} className=' even:bg- odd:bg-amber-50 odd:text-[#da5fda]'>
                <td className='px-5 md:px-10 lg:px-13 py-1.5 hidden md:block'>{i + 1}</td>
                <td className='px-5 md:px-10 lg:px-13 py-1.5'>{data.studentId.rollNo}</td>
                ...
                <td className='pl-7 pr-5 md:px-10 lg:px-13 py-1.5'>{data.marks.accuredMark}</td>
              </tr>
            ))}
          </tbody>
      </table>
